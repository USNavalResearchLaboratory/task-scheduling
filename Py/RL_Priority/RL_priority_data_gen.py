...
## Description
# This file runs a radar-like scheduler with 120 search tasks and 20 tracks
# The data is stored in "feat_record" which has dimensions (NumSteps, M, NUM_FEATS) where M = number of tasks (140)
#
...
#
# This data is saved and used to find the number of clusters in the state space
# Other considerations: The priorities are randomly assigned each iteration and N jobs are given to EST to schedule.
# Then those N tasks are added back to the stack
...
l_ex_remainder = np.zeros((NumSteps,len(alg_reprs)))
...
for alg_repr, alg_func, n_run in zip(alg_reprs, alg_funcs, alg_n_runs):
    ...
    for i_run in range(n_run):  # Perform new algorithm runs: Should never be more than 1 in this code
        ...
        for ii in np.arange(NumSteps): # Main Loop to evaluate schedulers
            ...
            priority_Idx = np.random.permutation(len(job))
            ...
            l_ex_remainder[ii,idx_alg] = eval_loss(job, t_max*np.ones(len(job)))
...
plt.plot(kl.elbow, sse[kl.elbow-2], marker='o', markersize=10, color="red")
...
plt.figure(3)
for jj in range(5):
    data_idx = kmeans.labels_ == jj
    plt.scatter(scaled_features[data_idx, 0], scaled_features[data_idx, 4], label='cluster = ' + str(jj))
plt.legend()
plt.show()
