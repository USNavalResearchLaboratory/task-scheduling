def training_with_n_dimensional_regression_function(number_of_tasks,features_indices,number_of_steps,F_training,F_testing,permutations,zero_cost_reward,episodes,alpha,gamma,epsilon):

    print("\n\nExecuting 'training_with_n_dimensional_regression.py' ...")
    ...
    from n_dimensional_regression_coefficients_results import n_dimensional_regression_coefficients_results_function
    ...
    Q = numpy.zeros([Q_number_of_elements,math.factorial(number_of_tasks)])
    ...
    for i in range(1,episodes+1):
        ...
        for j in range(len(F_training)-1):
            ...
            X = [1,r_state,w_state,l_state]

            B = n_dimensional_regression_coefficients_results_function(number_of_tasks,r_N_steps)

            Q_state = int(numpy.matmul(X,B))
            ...
            r_state_next = observations_to_state_release_times_function(number_of_tasks,r_continuous_next,r_N_steps)
            w_state_next = observations_to_state_weights_function(number_of_tasks,w_continuous_next,w_N_steps)
            l_state_next = observations_to_state_lengths_function(number_of_tasks,l_continuous_next,l_N_steps)

            X_next = [1,r_state_next,w_state_next,l_state_next]

            Q_state_next = int(numpy.matmul(X_next,B))

            maximum_next = numpy.max(Q[Q_state_next][:])
