...
if False: # Don't schedule Search
...
for alg_repr, alg_func, n_run in zip(alg_reprs, alg_funcs, alg_n_runs):
    ...
    for i_run in range(n_run):  # Perform new algorithm runs: Should never be more than 1 in this code
        ...
        for ii in np.arange(NumSteps): # Main Loop to evaluate schedulers
            ...
            features = np.empty((NUM_FEATS, M))
            for mm in range(len(job)):
                task = job[mm]
                features[0, mm] = np.array([task.t_release])
                features[1, mm] = np.array([task.duration])
                features[2, mm] = np.array([task.t_drop])
                features[3, mm] = np.array([task.l_drop])
                features[4, mm] = np.array([task.slope])
