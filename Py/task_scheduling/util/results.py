...
def struct_mean(array):
    array = iter_to_mean(array)
    data = tuple(array[name].mean() for name in array.dtype.names)
    return np.array(data, dtype=array.dtype)



# def evaluate_single(algorithms, tasks, ch_avail, solution_opt=None, verbose=0, plotting=0):
#
...
#
#     if plotting >= 1:
#         _, ax_gen = plt.subplots(2, 1, num='Scheduling Problem', clear=True)
#         plot_task_losses(tasks, ax=ax_gen[0])
#         scatter_loss_runtime(t_run_iter, l_ex_iter, ax=ax_gen[1])
#
...
def evaluate_single(algorithms, tasks, ch_avail, solution_opt=None, verbose=0, plotting=0):
    ...
    if solution_opt is not None and 'BB Optimal' not in algorithms['name']:
        _opt = np.array([('BB Optimal', None, 1)], dtype=[('name', '<U16'), ('func', object), ('n_iter', int)])
        algorithms = np.concatenate((_opt, algorithms))
...
#             solution_opt = None
#
#         l_ex_iter[i_gen], t_run_iter[i_gen] = evaluate_single(algorithms, tasks, ch_avail, solution_opt,
#                                                               verbose, plotting - 1)
#
#     return l_ex_iter, t_run_iter
...
#                 solution_opt = None
#
#             l_ex_iter[i_mc, i_gen], t_run_iter[i_mc, i_gen] = evaluate_single(algorithms, tasks, ch_avail, solution_opt,
#                                                                               verbose=0, plotting=0)
#
#     return l_ex_iter, t_run_iter
...
def evaluate_algorithms(algorithms, problem_gen, n_gen=1, solve=False, verbose=0, plotting=0, log_path=None):
    ...
    if solve and 'BB Optimal' not in algorithms['name']:
        _opt = np.array([('BB Optimal', None, 1)], dtype=[('name', '<U16'), ('func', object), ('n_iter', int)])
        algorithms = np.concatenate((_opt, algorithms))
    ...
    _array_mean = np.array([(np.nan,) * len(algorithms)] * n_gen,
                           dtype=[(alg['name'], float) for alg in algorithms])

    l_ex_mean, t_run_mean = _array_mean.copy(), _array_mean.copy()
    ...
    for i_gen, out_gen in enumerate(problem_gen(n_gen, solve, verbose)):
        ...
        # l_ex_iter[i_gen], t_run_iter[i_gen] = evaluate_single(algorithms, tasks, ch_avail, solution_opt, verbose - 1,
        #                                                       plotting - 1)
        l_ex_iter, t_run_iter = evaluate_single(algorithms, tasks, ch_avail, solution_opt, verbose - 1, plotting - 1)
        l_ex_mean[i_gen], t_run_mean[i_gen] = map(iter_to_mean, (l_ex_iter, t_run_iter))
...
def evaluate_algorithms_train(algorithms, train_args, problem_gen, n_gen=1, n_mc=1, solve=False, verbose=0, plotting=0,
    ...
    _array = np.array([(np.nan,) * len(algorithms)] * n_mc,
                      dtype=[(alg['name'], float) for alg in algorithms])

    l_ex_mc, t_run_mc = _array.copy(), _array.copy()
    ...
    for i_mc in range(n_mc):
        ...
        # l_ex_mc[i_mc], t_run_mc[i_mc] = l_ex_mean.mean(), t_run_mean.mean()
        l_ex_mc[i_mc], t_run_mc[i_mc] = struct_mean(l_ex_mean), struct_mean(t_run_mean)

    # Results
    if plotting >= 1:
        __, ax_results = plt.subplots(num='Results MC', clear=True)
        scatter_loss_runtime(t_run_mc, l_ex_mc,
                             ax=ax_results,
                             # ax_kwargs={'title': f'Performance, {problem_gen.n_tasks} tasks'}
                             )

    if solve:  # relative to B&B
        l_ex_mc_rel = l_ex_mc.copy()
        for name in algorithms['name']:
            l_ex_mc_rel[name] -= l_ex_mc['BB Optimal']
        ...
        if plotting >= 1:
            ...
            names_no_bb = algorithms['name'].tolist()
            names_no_bb.remove('BB Optimal')
            __, ax_results_rel_no_bb = plt.subplots(num='Results MC (Relative, opt excluded)', clear=True)
            scatter_loss_runtime(t_run_mc[names_no_bb], l_ex_mc_rel[names_no_bb],
                                 ax=ax_results_rel_no_bb,
                                 ax_kwargs={'ylabel': 'Excess Loss',
                                            # 'title': f'Relative performance, {problem_gen.n_tasks} tasks',
                                            }
    ...
    if verbose >= 1:  # TODO: move calc to _train func?
        _data = [[l_ex_mc[name].mean(), t_run_mc[name].mean()] for name in algorithms['name']]
        df = pd.DataFrame(_data, index=pd.CategoricalIndex(algorithms['name']), columns=['Loss', 'Runtime'])
        df_str = '\n' + df.to_markdown(tablefmt='github', floatfmt='.3f')
