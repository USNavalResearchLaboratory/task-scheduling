...
for Nindex = 1:length(Nvec)
    N = Nvec(Nindex);
    clear queueID metrics desiredRevisitRate JobRevistTime
    for TrackIndex = 1:length(NtrackVec)
        Ntrack = NtrackVec(TrackIndex);
        ...
        Data.ComputeTime(Nindex,TrackIndex,:) = time_vec;
        Data.Penalty(Nindex,TrackIndex,:) = -penalty_vec;
        
        
        figure(7); clf;
        AvgCost{Nindex,TrackIndex} = mean(loss_mc);
        for jj = 1:size(loss_mc,2)
            AvgCost{Nindex,TrackIndex}(jj) = mean( loss_mc( loss_mc(:,jj) > 0,jj));
        end
        AvgTime{Nindex,TrackIndex} = mean(t_run_mc)*1000;
        clf;
        hold all; grid on
        for jj = 1:length(approach_string)
            plot(AvgTime{Nindex,TrackIndex}(jj),AvgCost{Nindex,TrackIndex}(jj),shape(jj),'MarkerSize',10,'LineWidth',3)
        end
        % plot(time_vec(2),-penalty_vec(2),'x','MarkerSize',10,'LineWidth',3)
        % plot(time_vec(3),-penalty_vec(3),'s','MarkerSize',10,'LineWidth',3)
        legend(leg_str,'Location','best')
...
hold all
for jj = 1:length(approach_string)
   plot3(NtrackVec,Data.ComputeTime(:,:,jj), Data.Penalty(:,:,jj)) 
end
grid on
...
figure(81); clf;
plot(cell2mat(AvgTime),cell2mat(AvgCost))
ylabel('Cost')
xlabel('Computation Time (ms)')
title('Computation Time vs. Cost')
