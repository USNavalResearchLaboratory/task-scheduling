...
addpath(genpath(['.' filesep 'TaskSelectionSchedulingMultichannelRadar']))
...
for N = N_vec
    for monte = 1:MONTE
        ...
        [T,Tscheduled,Tdrop,NodeStats] = BBwithStats(data);
        ...
        if any( (Cost.BB(monte,cnt.N) - 1e-4)> [Cost.EST(monte,cnt.N)  Cost.EstSwap(monte,cnt.N) Cost.ED(monte,cnt.N) Cost.EdSwap(monte,cnt.N)] )
            keyboard
        end
...
FLAG.RandomSample = 0; % Otherwise group by problem index so problems aren't divided over test/train/validate boundary
if FLAG.RandomSample
    ...
    NtrainStart = 1;
    NtrainStop = find(ProblemIndex == ProblemIndex(Ntrain),1,'last');
...
layers = [
    ...
    convolution2dLayer([4 1],CNN_filters,'Name','conv2','Padding','Same')
    ...
    convolution2dLayer([2 2],CNN_filters,'Name','conv5','Padding','Same')
