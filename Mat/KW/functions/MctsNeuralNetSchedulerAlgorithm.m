...
while ~isempty(S.ND) % Proceed if base-node ND set is not empty
    ...
    for monte = 1:MONTE
        ...
        while ~isempty(NDprime)
            ...
            % Encode Features
            PF(1,:) = s_task(task_index);
            PF(2,:) = deadline_task(task_index);
            PF(3,:) = length_task(task_index);
            PF(4,:) = drop_task(task_index);
            PF(5,:) = w_task(task_index);
            
            % Encode Position in Tree
            PFtree = zeros(Np,Np);
            if ~isempty(Tprime)
                IND = sub2ind([Np Np],[1:length(Tprime)]',Tprime);            
                PFtree(IND) = 1;
            end
            ...
            PfStatus = zeros(3,Np);
            PfStatus(1,:) = 1;                    
            for nn = 1:length(Tprime)
                PfStatus(: , Tprime(nn) ) = [0; 0; 1]; % Infeasible Already Assigned
            end
            ...
            scores(Tprime(Tprime ~= 0)) = 0; % Remove Nodes from Scoring that have been assigned
            scores = scores/(sum(scores));            
            apriori = scores;
            
            % Selection
            RN = rand;
            TempIndex = find( RN > cumsum(apriori));
