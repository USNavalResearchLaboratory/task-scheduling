function [t_ex_opt,l_opt,t_run,X,Y] = fcn_BB_NN_linear_FAST(s_task,d_task,w_task,mode_stack,varargin)
...
for n = 1:N
    LB = LB + cost_linear(s_task(n),w_task(n),s_task(n));
    UB = UB + cost_linear(temp,w_task(n),s_task(n));
...
while (numel(S) ~= 1) || (numel(S(1).seq) ~= N)
    ...
    for n = seq_rem'
        ...
        %         Tprime = [B.seq; n]; % New Sequence
        %         ExecutionTime = B.t_ex;
        %         ExecutionTimeOffsetFlag = (diff([0; ExecutionTime(Tprime)]) >= 0);
        
        %         if all(ExecutionTimeOffsetFlag) % Checks for start-times dominance (All task should be in ascending execution time order, otherwise suboptimal scheduling)
        
        
        % Generate New Branch
        %         B_new = branch_update(B,n,l_task,s_task,d_task,w_task,t_drop,c_drop,B_new,curTime);
        B_new = branch_update_linear(B,n,w_task,s_task,d_task,B_new,curTime);
