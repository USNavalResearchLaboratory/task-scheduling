...
for device in tf.config.experimental.list_physical_devices("GPU"):
    tf.config.experimental.set_memory_growth(
        device, True
    )  # TODO: compatibility issue workaround
...
problem_gen = problem_gens.Dataset.load(
    "data/discrete_relu_c1t8", shuffle=False, repeat=True, rng=seed
)
...
model = keras.Sequential(
    [
        ...
        *layers,
        keras.layers.Dense(
            env.action_space.n, activation="softmax", kernel_initializer=_weight_init()
        ),
    ]
)
model.compile(
