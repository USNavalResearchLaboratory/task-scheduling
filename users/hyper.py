...
def objective(trial):
    ...
    hidden_dims = [trial.suggest_int("n_units_l{}".format(i), 10, 10000, log=True) for i in range(n_layers)]
    module = MultiNet.mlp(env, hidden_sizes_ch=[], hidden_sizes_tasks=[], hidden_sizes_joint=hidden_dims)
    ...
    loss_mean, t_run_mean = evaluate_algorithms_gen(algorithms, problem_gen, n_gen, n_gen_learn, solve=True,
                                                    verbose=1, plotting=0, log_path=log_path, img_path=img_path,
                                                    rng=seed)
